...
class PrettyDioLogger extends Interceptor {
  ...
  void onRequest(RequestOptions options,
    ...
    if (requestHeader) {
      ...
      requestHeaders['responseType'] =
          options.responseType.toString();
      requestHeaders['followRedirects'] =
      ...
      logPrint(
        (i >= 0 ? '║ ' : '') +
            msg.substring(
              i * maxWidth,
              math.min<int>(
                  i * maxWidth + maxWidth, msg.length),
            ),
      );
    }
  }
    ...
    data.keys
      ...
      if (value is Map) {
        if (compact && _canFlattenMap(value)) {
          logPrint(
              '║${_indent(localTabs)} $key: $value${!isLast ? ',' : ''}');
        } else {
        ...
        if (compact && _canFlattenList(value)) {
          logPrint(
              '║${_indent(localTabs)} $key: ${value.toString()}');
        } else {
          logPrint('║${_indent(localTabs)} $key: [');
          _printList(value, tabs: localTabs);
          logPrint(
              '║${_indent(localTabs)} ]${isLast ? '' : ','}');
        }
      } else {
        ...
        if (msg.length + indent.length > linWidth) {
          ...
          logPrint(
              '║${_indent(localTabs)} $key: $msg${!isLast ? ',' : ''}');
        }
      }
    });
  ...
  bool _canFlattenMap(Map map) {
    return map.values
            .where(
                (dynamic val) => val is Map || val is List)
            .isEmpty &&
